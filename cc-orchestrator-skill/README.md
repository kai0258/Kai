...
```bash
git clone https://github.com/kai0258/cc-orchestrator.git
cp -r cc-orchestrator ~/.hermes/skills/
```
...
# Claude Code
mkdir -p ~/.claude/skills && cp -r cc-orchestrator ~/.claude/skills/
```
...
**Q: 和 task-os 是什么关系？**
A: task-os 是有代码的自动化工具（引擎），cc-orchestrator 是纯方法论（手册）。task-os 自动跑但不支持并行；cc-orchestrator 支持并行但需要手动执行。详见 SKILL.md。
